...
export class CustomersController {
  ...
  @Post('addresses')
  async addAddress(@Body() body: { email: string; address: any }, @Res() res: Response) {
    try {
      const addresses = await this.customersService.addAddress(body.email, body.address);
      return res.status(HttpStatus.OK).json({ success: true, addresses });
    } catch (err) {
      console.error(err);
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ success: false, message: err.message });
  ...
  async updateAddress(@Body() body: { email: string; addressId: string; updatedFields: any }, @Res() res: Response) {
    try {
      const addresses = await this.customersService.updateAddress(body.email, body.addressId, body.updatedFields);
      return res.status(HttpStatus.OK).json({ success: true, addresses });
    } catch (err) {
      console.error(err);
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ success: false, message: err.message });
  ...
  async deleteAddress(@Body() body: { email: string; addressId: string }, @Res() res: Response) {
    try {
      const addresses = await this.customersService.deleteAddress(body.email, body.addressId);
      return res.status(HttpStatus.OK).json({ success: true, addresses });
    } catch (err) {
      console.error(err);
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ success: false, message: err.message });
